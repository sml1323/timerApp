...
export async function createTopic(input: CreateTopicInput): Promise<Result<Topic>> {
  try {
    ...
    // 2. 중복 이름 확인 (활성 주제만)
    const duplicates = await select<TopicRow>(
      'SELECT id FROM topics WHERE name = $1 AND is_archived = 0',
      [name],
    );
    ...
    // 4. INSERT
    await execute(
      'INSERT INTO topics (id, name, is_archived, created_at_ms, updated_at_ms) VALUES ($1, $2, $3, $4, $5)',
      [id, name, 0, now, now],
    );
 ...
 */
export async function findAllTopics(includeArchived: boolean = false): Promise<Result<Topic[]>> {
  try {
    const query = includeArchived
      ? 'SELECT * FROM topics ORDER BY created_at_ms DESC'
      : 'SELECT * FROM topics WHERE is_archived = 0 ORDER BY created_at_ms DESC';

    const rows = await select<TopicRow>(query);
...
export async function updateTopic(id: string, input: UpdateTopicInput): Promise<Result<Topic>> {
  try {
    ...
    const duplicates = await select<TopicRow>(
      'SELECT id FROM topics WHERE name = $1 AND is_archived = 0 AND id != $2',
      [name, id],
    );
    if (duplicates.length > 0) {
      return err(ERROR_CODES.VALIDATION_ERROR, 'A topic with this name already exists');
    ...
    const now = Date.now();
    await execute(
      'UPDATE topics SET name = $1, updated_at_ms = $2 WHERE id = $3',
      [name, now, id],
    );
...
export async function archiveTopic(id: string): Promise<Result<Topic>> {
  try {
    ...
    const now = Date.now();
    await execute(
      'UPDATE topics SET is_archived = 1, updated_at_ms = $1 WHERE id = $2',
      [now, id],
    );
