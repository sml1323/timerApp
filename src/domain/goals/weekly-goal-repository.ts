...
export async function createWeeklyGoal(input: CreateWeeklyGoalInput): Promise<Result<WeeklyGoal>> {
  try {
    ...
    if (rows.length === 0) {
      return err(ERROR_CODES.PERSISTENCE_ERROR, 'Failed to load the weekly goal after creation');
    }
    ...
    return err(
      ...
      `An error occurred while creating the weekly goal: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
...
export async function updateWeeklyGoal(id: string, input: UpdateWeeklyGoalInput): Promise<Result<WeeklyGoal>> {
  try {
    ...
    const rows = await select<WeeklyGoalRow>('SELECT * FROM weekly_goals WHERE id = $1', [id]);
    if (rows.length === 0) {
      return err(ERROR_CODES.PERSISTENCE_ERROR, 'Failed to load the weekly goal after updating it');
    }
    ...
    return err(
      ERROR_CODES.PERSISTENCE_ERROR,
      `An error occurred while updating the weekly goal: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
...
export async function findByTopicAndWeek(topicId: string, weekStartAtMs: number): Promise<Result<WeeklyGoal | null>> {
  try {
    ...
    return err(
      ERROR_CODES.PERSISTENCE_ERROR,
      `An error occurred while loading the weekly goal: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
...
export async function findAllByWeek(weekStartAtMs: number): Promise<Result<WeeklyGoal[]>> {
  try {
    ...
    return err(
      ERROR_CODES.PERSISTENCE_ERROR,
      `An error occurred while loading weekly goals: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
