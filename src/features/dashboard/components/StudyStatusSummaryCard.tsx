...
export function StudyStatusSummaryCard({ data, isLoading, error }: StudyStatusSummaryCardProps) {
  ...
  if (error) {
    return (
      <div className={styles.card} role="region" aria-label="학습 상태 요약">
        <p className={styles.errorText}>학습 상태를 불러올 수 없습니다.</p>
      </div>
    );
  }
    ...
    return (
      ...
      >
        <div className={styles.emptyState}>
          ...
          <p className={styles.emptyHint}>주제를 선택하고 첫 세션을 시작해보세요.</p>
        </div>
      </button>
    );
  }
      ...
      <div className={styles.cardContent}>
        <div className={styles.statsArea}>
          <div className={styles.statBlock}>
            ...
            <span className={styles.statSub}>{data.todaySessionCount}개 세션</span>
          </div>
          ...
          <div className={styles.statBlock}>
            ...
            <span className={styles.statSub}>{data.weeklySessionCount}개 세션</span>
          </div>
        </div>
            ...
            <div className={styles.goalDetails}>
              {progressState === 'achieved' ? (
                ...
                <>
                  ...
                  >
                    남은 시간: {formatMinutes(data.remainingMinutes)}
                  </span>
                </>
              )}
              <span className={styles.goalCount}>
                {data.achievedGoalCount}/{data.goalCount} 목표 달성
              </span>
            </div>
          </div>
        ) : (
          <div className={styles.noGoalArea}>
            <p className={styles.noGoalText}>주간 목표를 설정하고 진행률을 추적하세요.</p>
          </div>
        )}
      </div>
    </button>
